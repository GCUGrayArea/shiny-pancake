 ...
 */
export async function getCulturalHints(
  messageId: string,
): Promise<ContextHint[]> {
  // Check cache first
...
export async function markHintAsSeen(hintId: string): Promise<void> {
  const db = getDatabase();
  await db.runAsync(`UPDATE cultural_hints SET seen = 1 WHERE id = ?`, [
    hintId,
  ]);
...
export async function deleteCulturalHints(messageId: string): Promise<void> {
  const db = getDatabase();
  await db.runAsync(`DELETE FROM cultural_hints WHERE messageId = ?`, [
    messageId,
  ]);
