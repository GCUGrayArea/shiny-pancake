...
import { Reply, ReplyType, UserStyleProfile, LanguageCode, SmartReplyOptions } from '../types';
...
export async function generateSmartReplies(
  ...
  const prompt = buildReplyPrompt(recentMessages, userProfile, count, targetLanguage);

  try {
    console.log(`🤖 Smart Reply: Generating replies in ${options.targetLanguage || userProfile.languageMixing.primary}`);
    ...
    console.log('🤖 Smart Reply: Received response from OpenAI');

    // Parse the response
    const targetLang = options.targetLanguage || userProfile.languageMixing.primary;
    const replies = parseReplyResponse(response, targetLang);

    console.log(`🤖 Smart Reply: Generated ${replies.length} replies in ${targetLang}`);

    // Return requested number of replies
    return replies.slice(0, count);
  } catch (error) {
    console.error('❌ Smart Reply: Failed to generate:', error);
    console.error('❌ Smart Reply: Error details:', error instanceof Error ? error.message : String(error));
...
function buildStyleDescription(profile: UserStyleProfile): string {
  ...
  if (profile.emojiUsage.frequency > 0.5) {
    parts.push(`- I frequently use emojis (favorites: ${profile.emojiUsage.favorites.join(' ')})`);
 ...
 */
function parseReplyResponse(response: string, defaultLanguage: LanguageCode): Reply[] {
  try {
...
function generateDefaultReplies(language: LanguageCode): Reply[] {
  const defaults: Record<string, Reply[]> = {
    en: [
      ...
      { text: 'What do you think?', type: 'question', language: 'en', confidence: 0.5 },
      { text: 'Tell me more', type: 'continue', language: 'en', confidence: 0.5 },
    ],
    es: [
      { text: '¡Suena bien!', type: 'agree', language: 'es', confidence: 0.5 },
      { text: '¿Qué piensas?', type: 'question', language: 'es', confidence: 0.5 },
      { text: 'Cuéntame más', type: 'continue', language: 'es', confidence: 0.5 },
    ],
    fr: [
      { text: 'Ça a l\'air bien!', type: 'agree', language: 'fr', confidence: 0.5 },
      { text: 'Qu\'en penses-tu?', type: 'question', language: 'fr', confidence: 0.5 },
      { text: 'Dis-m\'en plus', type: 'continue', language: 'fr', confidence: 0.5 },
    ],
  };
 ...
 */
function getCacheKey(chatId: string, lastMessageId: string, language?: LanguageCode): string {
 ...
 */
export function getCachedReplies(chatId: string, lastMessageId: string, language?: LanguageCode): Reply[] | null {
 ...
 */
export function cacheReplies(chatId: string, lastMessageId: string, replies: Reply[], language?: LanguageCode): void {
  const key = getCacheKey(chatId, lastMessageId, language);
  replyCache.set(key, {
    replies,
    timestamp: Date.now(),
